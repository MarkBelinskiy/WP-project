<?php
add_image_size('photo_moto', 713, 420, 1 );
add_image_size('photo_blog', 198, 394, 1 );
add_image_size('photo_about', 265, 415, 1 );
...
function nealine($str) {
	$arr = explode(" ", $str);
	$sep = ceil(count($arr)/2);
	$title = "";
	$i = 1;
	foreach($arr as $one) {
		if($i == $sep) {
...
function rex($one){
	$ex = explode("|", $one);
	if(count($ex) > 1) {
		return '<span><i class="fa fa-'.$ex[0].'" aria-hidden="true"></i>'.$ex[1].'</span>';
	}
	return '<span>'.$one.'</span>';
}

function tes($str) {
	return implode("", array_map('rex', preg_split("#<br */?>#is", preg_replace("#\r?\n#is", "", $str))));
}
...
class Kama_Breadcrumbs {
	...
	function get_crumbs( $sep, $l10n, $args ){
		...
		$f = apply_filters('kama_breadcrumbs', $out, $sep, $loc, $arg );
		
		preg_match_all('#<a href="(.*?)".*?>.*?</a>#is', $f, $links);
		$lold = array_pop(array_pop($links));
		$lnew = preg_replace("#(.*?/goods/)(.*?)/#is", "$1#.$2", $lold);

		return str_replace($lold, $lnew, apply_filters('kama_breadcrumbs', $out, $sep, $loc, $arg ));
	}
